    ...
    with t1:
        if not st.session_state.final_cv:
            with st.spinner("⏳ Translating & Writing Resume..."):
                ...
                # --- EDUCATION FIX: Single Line Format ---
                edu_block = ""
                if any(st.session_state.cv_data.get(k) for k in ['university', 'college', 'degree', 'grad_year']):
                    parts = []
                    if st.session_state.cv_data.get('degree'): parts.append(st.session_state.cv_data['degree'])
                    if st.session_state.cv_data.get('college'): parts.append(st.session_state.cv_data['college'])
                    if st.session_state.cv_data.get('university'): parts.append(st.session_state.cv_data['university'])
                ...
                **CRITICAL RULES:** 1. **TRANSLATE ALL INPUT TO ENGLISH.** (e.g. "الجيزة" -> "Giza", "طالب" -> "Student").
                ...
                5. Education MUST be in one line like: "Bachelor, Faculty of X, University | 2024"
